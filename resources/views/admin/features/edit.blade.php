        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">{{__('admin.update') . ' ' . __('admin.feature')}}</h4>
                </div>
                <div class="card-content">
                    <div class="card-body">
                        <form  method="POST" action="{{route('admin.features.update' , ['id' => $feature->id])}}" class="store form-horizontal" novalidate>
                            @csrf
                            @method('PUT')
                            <div class="form-body">
                                <div class="row">
                                    @foreach (languages() as $lang)
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <div class="controls">
                                                    <input type="text" name="name[{{$lang}}]" value="{{$feature->getTranslations('name')[$lang]}}" class="form-control" placeholder="{{__('site.write') . __('site.name_'.$lang)}}"/>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
